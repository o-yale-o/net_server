...
/******************************************************************************************
函数原型: 
功能描述: 来数据时候的处理，当连接上有数据来的时候，本函数会被 ngx_epoll_process_events() 所调用  ,官方的类似函数为 ngx_http_wait_request_handler();
参数说明:   名称            类型                说明

返 回 值: 
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 09时52分58秒
修改记录: 
...
{  
    ...
    ssize_t lRet = ReadData(pConn, pConn->pcRecvBuff, pConn->iRecvLen ); 
    if( lRet <= 0 )  
    {
    ...
    // 收到了一些字节（>0）：判断收了多少数据
    if(pConn->cCurrStat == PKG_HD_INIT) //连接建立起来时肯定是这个状态，因为在ngx_get_connection()中已经把curStat成员赋值成_PKG_HD_INIT了
    {        
        if(lRet == m_iLenPkgHeader)//正好收到完整包头，这里拆解包头
        {   
        ...
        else
		{
            ...
            pConn->cCurrStat    = PKG_HD_RECVING;          //接收包头中，包头不完整，继续接收包头中	
            pConn->pcRecvBuff   = pConn->pcRecvBuff + lRet; //注意收后续包的内存往后走
            pConn->iRecvLen     = pConn->iRecvLen - lRet;   //要收的内容当然要减少，以确保只收到完整的包头先
        } //end  if(lRet == m_iLenPkgHeader)
    } 
    else if(pConn->cCurrStat == PKG_HD_RECVING) //接收包头中，包头不完整，继续接收中，这个条件才会成立
    {
        if(pConn->iRecvLen == lRet) //要求收到的宽度和我实际收到的宽度相等
        {
            //包头收完整了
            ngx_wait_request_handler_proc_p1(pConn,isflood); //那就调用专门针对包头处理完整的函数去处理把。
        }
		...
		{
			//包头还是没收完整，继续收包头
            //pConn->cCurrStat        = PKG_HD_RECVING;                 //没必要
            pConn->pcRecvBuff       = pConn->pcRecvBuff + lRet;              //注意收后续包的内存往后走
            pConn->iRecvLen       = pConn->iRecvLen - lRet;              //要收的内容当然要减少，以确保只收到完整的包头先
        }
    }
    else if(pConn->cCurrStat == PKG_BD_INIT) 
    {
        //包头刚好收完，准备接收包体
        if(lRet == pConn->iRecvLen)
        {
            //收到的宽度等于要收的宽度，包体也收完整了
            if(m_iIsCheckFloodAttack == 1) 
            {
                //Flood攻击检测是否开启
		...
		{
			//收到的宽度小于要收的宽度
			pConn->cCurrStat = PKG_BD_RECVING;
			pConn->pcRecvBuff = pConn->pcRecvBuff + lRet;
			pConn->iRecvLen = pConn->iRecvLen - lRet;
		}
    }
    else if(pConn->cCurrStat == PKG_BD_RECVING) 
    {
        //接收包体中，包体不完整，继续接收中
        if(pConn->iRecvLen == lRet)
        {
            //包体收完整了
            if(m_iIsCheckFloodAttack == 1) 
            {
    ...
    }  //end if(c->cCurrStat == PKG_HD_INIT)

    if(isflood == true)
    {
        //客户端flood服务器，则直接把客户端踢掉
        //LOG_STDERR1(errno,"发现客户端flood，干掉该客户端!");
        KickConnection(pConn);
    }

    return;
}// end OnRead(lpngx_connection_t pConn)

/******************************************************************************************
函数原型: 
功能描述: 接收数据专用函数--引入这个函数是为了方便，如果断线，错误之类的，这里直接 释放连接池中连接，然后直接关闭socket，以免在其他函数中还要重复的干这些事
参数说明:   名称            类型                说明
            pConn           lpngx_connection_t  连接池中相关连接
            buff            char*               接收数据的缓冲区
            buflen          ssize_t             要接收的数据大小
                                                注： ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
返 回 值: 返回-1，则是有问题发生并且在这里把问题处理完毕了，调用本函数的调用者一般是可以直接return
        返回>0，则是表示实际收到的字节数
依 赖 于: 
被引用于: void CSocekt::OnRead(lpngx_connection_t pConn)
创建日期: 2020年09月11日 09时53分10秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
ssize_t CSocekt::ReadData(lpngx_connection_t pConn,char *buff,ssize_t buflen)  
{
    ssize_t n;
    ...
    if(n < 0) //这被认为有错误发生
    {
        ...
        else
        {
            ...
            {
                LOG_STDERR1(errno,"发生错误，我打印出来看看是啥错误！");  //正式运营时可以考虑这些日志打印去掉
            }
        } 
...
/******************************************************************************************
函数原型: 
功能描述: 包头收完整后的处理，我们称为包处理阶段1【p1】
参数说明:   名称            类型                说明
            pConn           lpngx_connection_t  连接池中相关连接
            isflood         bool&
返 回 值: 
依 赖 于: 
被引用于: CSocekt::OnRead(lpngx_connection_t pConn)
创建日期: 2020年09月11日 09时55分38秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
...
{    
    ...
    pPkgHeader = (LPCOMM_PKG_HEADER)pConn->szDataHeadInfo; //正好收到包头时，包头信息肯定是在dataHeadInfo里；

    unsigned short iPkgLen; 
    iPkgLen = ntohs(pPkgHeader->pkgLen);  //注意这里网络序转本机序，所有传输到网络上的2字节数据，都要用htons()转成网络序，所有从网络上收到的2字节数据，都要用ntohs()转成本机序
                                                //ntohs/htons的目的就是保证不同操作系统数据之间收发的正确性，【不管客户端/服务器是什么操作系统，发送的数字是多少，收到的就是多少】
                                                //不明白的同学，直接百度搜索"网络字节序" "主机字节序" "c++ 大端" "c++ 小端"
    //恶意包或者错误包的判断
    if(iPkgLen < m_iLenPkgHeader) 
    {
        ...
        pConn->cCurrStat = PKG_HD_INIT;      
        pConn->pcRecvBuff = pConn->szDataHeadInfo;
        pConn->iRecvLen = m_iLenPkgHeader;
    }
    else if(iPkgLen > (_PKG_MAX_LENGTH-1000))   //客户端发来包居然说包长度 > 29000?肯定是恶意包
    {
    ...
    else
    {
        ...
        char *pTmpBuffer  = (char *)pMemory->AllocMemory(m_iLenMsgHeader + iPkgLen,false); //分配内存【长度是 消息头长度  + 包头长度 + 包体长度】，最后参数先给false，表示内存不需要memset;        
        pConn->pcRecvMemery = pTmpBuffer;  //内存开始指针
        ...
        {
            //开始收包体，注意我的写法
            pConn->cCurrStat = PKG_BD_INIT;                   //当前状态发生改变，包头刚好收完，准备接收包体	    
            pConn->pcRecvBuff = pTmpBuffer + m_iLenPkgHeader;  //pTmpBuffer指向包头，这里 + m_iLenPkgHeader后指向包体 weizhi
            pConn->iRecvLen = iPkgLen - m_iLenPkgHeader;    //e_pkgLen是整个包【包头+包体】大小，-m_iLenPkgHeader【包头】  = 包体
        }                       
    }  //end if(iPkgLen < m_iLenPkgHeader) 
...
/******************************************************************************************
函数原型: 
功能描述: 收到一个完整包后的处理【plast表示最后阶段】，放到一个函数中，方便调用
参数说明:   名称            类型                说明
            pConn           lpngx_connection_t  连接池中相关连接
            isflood         bool&
返 回 值: 
...
void CSocekt::ngx_wait_request_handler_proc_plast(lpngx_connection_t pConn,bool &isflood)
{
    ...
    //inMsgRecvQueue(c->pcRecvMemery,irmqc); //返回消息队列当前信息数量irmqc，是调用本函数后的消息队列中消息数量
    //激发线程池中的某个线程来处理业务逻辑
    //g_ThreadPool.Call(irmqc);

    if(isflood == false)
    {
        g_ThreadPool.inMsgRecvQueueAndSignal(pConn->pcRecvMemery); //入消息队列并触发线程处理消息
    }
    else
    {
        //对于有攻击倾向的恶人，先把他的包丢掉
        CMemory *pMemory = CMemory::GetInstance();
        pMemory->FreeMemory(pConn->pcRecvMemery); //直接释放掉内存，根本不往消息队列入
    }

    pConn->pcRecvMemery = NULL;
    pConn->cCurrStat         = PKG_HD_INIT;     //收包状态机的状态恢复为原始态，为收下一个包做准备                    
    pConn->pcRecvBuff        = pConn->szDataHeadInfo;  //设置好收包的位置
...
依 赖 于: 
被引用于: void CSocekt::OnWrite(lpngx_connection_t pConn)
创建日期: 2020年09月11日 09时57分14秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
ssize_t CSocekt::WriteData(lpngx_connection_t c,char *buff,ssize_t size)  //ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
{
    ...
    for ( ;; )
    {
        ...
        if(n == 0)
        {
            ...
            //连接断开epoll会通知并且 ReadData()里会处理，不在这里处理
            return 0;
        }
        ...
        if(errno == EINTR) 
        {
            ...
            LOG_STDERR1(errno,"send()失败!");  //打印个日志看看啥时候出这个错误
            //其他不需要做什么，等下次for循环吧            
        }
        else
        {
...
/******************************************************************************************
函数原型: 
功能描述: 响应数据可写(时epoll通知我们的)事件
        能走到这里，数据就是没法送完毕，要继续发送
参数说明:   名称            类型                说明

返 回 值: 
依 赖 于: 
被引用于: int CSocekt::ProcessEpollEvents(int timer)
          void* CSocekt::ServerSendQueueThread(void* pvThreadData)
创建日期: 2020年09月11日 09时58分19秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
...
/******************************************************************************************
函数原型: 
功能描述: 消息处理线程主函数，专门处理各种接收到的TCP消息
参数说明:   名称            类型                说明
            pcMsgBuf         char*               发送过来的消息缓冲区，消息本身是自解释的，通过包头可以计算整个包长
                                                消息本身格式【消息头+包头+包体】 
返 回 值: 
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 09时58分46秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void CSocekt::ProcessClientRequest(char *pcMsgBuf)
{   
    return;
}
