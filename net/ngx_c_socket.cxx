...
{
    //配置相关
    m_iWorkerMaxConnection = 1;      //epoll连接最大项数
    m_iListenPortNum = 1;         //监听一个端口
    m_iRecyConnectionWaitSeconds = 60; //等待这么些秒后才回收连接
    ...
    //各种队列相关
    m_iListSendBuffCount    = 0;     //发消息队列大小
    m_iTotolRecyConnection  = 0;     //待释放连接队列大小
    m_lTimeMaxCurrSize      = 0;     //当前计时队列尺寸
    m_timeValue             = 0;     //当前计时队列头部的时间值
    m_iDiscardSendPkgCount  = 0;     //丢弃的发送数据包数量

    //在线用户相关
    m_iOnlineUserCount        = 0;     //在线用户数量统计，先给0  
    m_timeLastPrint           = 0;     //上次打印统计信息的时间，先给0
    return;	
}

/******************************************************************************************
函数原型: 
功能描述: 初始化函数【fork()子进程之前干这个事】
参数说明:   名称            类型                说明

返 回 值: 成功返回true，失败返回false
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时00分07秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
...
/******************************************************************************************
函数原型: 
功能描述: 子进程中才需要执行的初始化函数
参数说明:   名称            类型                说明

返 回 值: 
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时00分28秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
bool CSocekt::InitializeSubProc()
{
    //发消息互斥量初始化
    if(pthread_mutex_init(&m_mutexListSendBuff, NULL)  != 0)
    {        
       LOG_STDERR("pthread_mutex_init(&m_mutexListSendBuff)失败.");
        return false;    
    }
    //连接相关互斥量初始化
    if(pthread_mutex_init(&m_mutexConnection, NULL)  != 0)
    {
       LOG_STDERR("pthread_mutex_init(&m_mutexConnection)失败.");
        return false;    
    }    
    //连接回收队列相关互斥量初始化
    if(pthread_mutex_init(&m_mutexRecyConnQueue, NULL)  != 0)
    {
       LOG_STDERR("pthread_mutex_init(&m_mutexRecyConnQueue)失败.");
        return false;    
    } 
    //和时间处理队列有关的互斥量初始化
    if(pthread_mutex_init(&m_mutexTimeMap, NULL)  != 0)
    {
       LOG_STDERR("pthread_mutex_init(&m_mutexTimeMap)失败.");
        return false;    
    }
    ...
    if(sem_init(&m_semEventSendBuff,0,0) == -1)
    {
       LOG_STDERR("sem_init(&m_semEventSendBuff,0,0)失败.");
        return false;
    }
    ...
    m_vectorThread.push_back(pSendQueue = new ThreadItem(this));                         //创建 一个新线程对象 并入到容器中 
    iErr = pthread_create(&pSendQueue->m_hThread, NULL, ServerSendQueueThread,pSendQueue); //创建线程，错误不返回到errno，一般返回错误码
    if(iErr != 0)
    {
       LOG_STDERR("pthread_create(ServerSendQueueThread)失败.");
        return false;
    }

    //---
    ThreadItem *pRecyconn;    //专门用来回收连接的线程
    m_vectorThread.push_back(pRecyconn = new ThreadItem(this)); 
    iErr = pthread_create(&pRecyconn->m_hThread, NULL, ServerRecyConnectionThread,pRecyconn);
    if(iErr != 0)
    {
       LOG_STDERR("pthread_create(ServerRecyConnectionThread)失败.");
        return false;
    }

    if(m_iIsOpenKickTimer == 1)  //是否开启踢人时钟，1：开启   0：不开启
    {
        ThreadItem *pTimemonitor;    //专门用来处理到期不发心跳包的用户踢出的线程
        m_vectorThread.push_back(pTimemonitor = new ThreadItem(this)); 
        iErr = pthread_create(&pTimemonitor->m_hThread, NULL, ServerTimerQueueMonitorThread,pTimemonitor);
        if(iErr != 0)
        {
           LOG_STDERR("pthread_create(ServerTimerQueueMonitorThread)失败.");
            return false;
        }
    }
...
CSocekt::~CSocekt()
{
	...
	for(pos = m_listListenSocket.begin(); pos != m_listListenSocket.end(); ++pos) //vector
	{		
...
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时00分49秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void CSocekt::ShutdownSubProc()
{
    //(1)把干活的线程停止掉，注意 系统应该尝试通过设置 g_iStopEvent = 1来 开始让整个项目停止
    //(2)用到信号量的，可能还需要调用一下sem_post
    if(sem_post(&m_semEventSendBuff)==-1)  //让ServerSendQueueThread()流程走下来干活
    {
        LOG_STDERR("CSocekt::ShutdownSubProc()中sem_post(&m_semEventSendBuff)失败.");
    }
    ...
    //(4)多线程相关    
    pthread_mutex_destroy(&m_mutexConnection);          //连接相关互斥量释放
    pthread_mutex_destroy(&m_mutexListSendBuff);    //发消息互斥量释放    
    pthread_mutex_destroy(&m_mutexRecyConnQueue);       //连接回收队列相关的互斥量释放
    pthread_mutex_destroy(&m_mutexTimeMap);           //时间处理队列相关的互斥量释放
    sem_destroy(&m_semEventSendBuff);                  //发消息相关线程信号量释放
}
...
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时00分58秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void CSocekt::ClearSendBuffList()
{
	char * sTmpMempoint;
	CMemory *pMemory = CMemory::GetInstance();
	
	while(!m_listSendBuff.empty())
	{
		sTmpMempoint = m_listSendBuff.front();
		m_listSendBuff.pop_front(); 
...
void CSocekt::ReadConf()
{
    ...
    m_iWorkerMaxConnection      = pConfig->GetIntDefault("worker_connections",m_iWorkerMaxConnection);              //epoll连接的最大项数
    m_iListenPortNum         = pConfig->GetIntDefault("ListenPortCount",m_iListenPortNum);                    //取得要监听的端口数量
    m_iRecyConnectionWaitSeconds  = pConfig->GetIntDefault("Sock_RecyConnectionWaitTime",m_iRecyConnectionWaitSeconds); //等待这么些秒后才回收连接

    m_iIsOpenKickTimer         = pConfig->GetIntDefault("Sock_WaitTimeEnable",0);                                //是否开启踢人时钟，1：开启   0：不开启
...
/******************************************************************************************
函数原型: 
功能描述: 监听端口【支持多个端口】，这里遵从nginx的函数命名
...
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时01分23秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
bool CSocekt::NgxOpenListeningSockets()
{    
    int                 iSocket;
    struct sockaddr_in  serv_addr;  //服务器的地址结构体
    int                 iPort;      //端口
    char                szTmp[100]; //临时字符串 
    ...
    //中途用到一些配置信息
    CConfig *pConfig = CConfig::GetInstance();
    for(int i = 0; i < m_iListenPortNum; i++) //要监听这么多个端口
    {        
        //参数1：AF_INET：使用ipv4协议，一般就这么写
        //参数2：SOCK_STREAM：使用TCP，表示可靠连接【相对还有一个UDP套接字，表示不可靠连接】
        //参数3：给0，固定用法，就这么记
        iSocket = socket(AF_INET,SOCK_STREAM,0); //系统函数，成功返回非负描述符，出错返回-1
        if(iSocket == -1)
        {
            LOG_STDERR1(errno,"socket()失败,i=%d.",i);
        ...
        int iReuseAddr = 1;  //1:打开对应的设置项
        if(setsockopt(iSocket,SOL_SOCKET, SO_REUSEADDR,(const void *) &iReuseAddr, sizeof(iReuseAddr)) == -1)
        {
            LOG_STDERR1(errno,"setsockopt(SO_REUSEADDR)失败,i=%d.",i);
            close(iSocket); //无需理会是否正常执行了                                                  
            return false;
        }
        ...
        if (setsockopt(iSocket, SOL_SOCKET, SO_REUSEPORT,(const void *) &reuseport, sizeof(int))== -1) //端口复用需要内核支持
        {
            ...
            LOG_STDERR1(errno,"setsockopt(SO_REUSEPORT)失败",i);
        }
        //else
        //{
        ...
        {                
            LOG_STDERR1(errno,"SetNonBlocking()失败,i=%d.",i);
            close(iSocket);
            return false;
        }
        ...
        {
            LOG_STDERR1(errno,"listen()失败,i=%d.",i);
            close(iSocket);
            return false;
        }

        //可以，放到列表里来
        lpngx_listening_t pNewItem = new ngx_listening_t; //千万不要写错，注意前边类型是指针，后边类型是一个结构体
        memset(pNewItem,0,sizeof(ngx_listening_t));      //注意后边用的是 ngx_listening_t而不是lpngx_listening_t
        pNewItem->iPort = iPort;                          //记录下所监听的端口号
        pNewItem->fd   = iSocket;                          //套接字木柄保存下来   
        LOG_INFO("监听端口%d...", iPort);
        m_listListenSocket.push_back(pNewItem);          //加入到队列中
    } //end for(int i = 0; i < m_iListenPortNum; i++)    
    if(m_listListenSocket.size() <= 0)  //不可能一个端口都不监听吧
    {
        return false;
    }
...
/******************************************************************************************
函数原型: 
功能描述: 设置socket连接为非阻塞模式【这种函数的写法很固定】：非阻塞，概念在五章四节讲解的非常清楚【不断调用，不断调用这种：拷贝数据的时候是阻塞的】
参数说明:   名称            类型                说明

返 回 值: 
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时01分43秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
bool CSocekt::SetNonBlocking(int sockfd) 
{    
    int iNoblockFlag = 1; //0：清除，1：设置  
    if(ioctl(sockfd, FIONBIO, &iNoblockFlag ) == -1) //FIONBIO：设置/清除非阻塞I/O标记：0：清除，1：设置
    {
        return false;
    }
    return true;
    ...
    if(opts < 0) 
    {
        LOG_STDERR1(errno,"CSocekt::SetNonBlocking()中fcntl(F_GETFL)失败.");
        return false;
    }
    opts |= O_NONBLOCK; //把非阻塞标记加到原来的标记上，标记这是个非阻塞套接字【如何关闭非阻塞呢？opts &= ~O_NONBLOCK,然后再F_SETFL一下即可】
    if(fcntl(sockfd, F_SETFL, opts) < 0) 
    {
        LOG_STDERR1(errno,"CSocekt::SetNonBlocking()中fcntl(F_SETFL)失败.");
        return false;
    }
    return true;
    */
...
/******************************************************************************************
函数原型: 
功能描述: 关闭socket，什么时候用，我们现在先不确定，先把这个函数预备在这里
参数说明:   名称            类型                说明

返 回 值: 
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时01分53秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void CSocekt::NgxCloseListeningSockets()
{
    for(int i = 0; i < m_iListenPortNum; i++) //要关闭这么多个监听端口
    {  
        //LogStdErr(0,"端口是%d,socketid是%d.",m_listListenSocket[i]->iPort,m_listListenSocket[i]->fd);
        close(m_listListenSocket[i]->fd);
        LOG_INFO("关闭监听端口%d!",m_listListenSocket[i]->iPort);
    }//end for(int i = 0; i < m_iListenPortNum; i++)
    return;
}
...
/******************************************************************************************
函数原型: 
功能描述: 将一个待发送消息入到发消息队列中
参数说明:   名称            类型                说明

返 回 值: 
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时02分02秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void CSocekt::PushData2SendBuff(char *pcData) 
{
    CMemory *pMemory = CMemory::GetInstance();

    CLock lock(&m_mutexListSendBuff);  //互斥量

    //发送消息队列过大也可能给服务器带来风险
    if(m_iListSendBuffCount > 50000)
    {
        //发送队列过大，比如客户端恶意不接受数据，就会导致这个队列越来越大
        //那么可以考虑为了服务器安全，干掉一些数据的发送，虽然有可能导致客户端出现问题，但总比服务器不稳定要好很多
        m_iDiscardSendPkgCount++;
        pMemory->FreeMemory(pcData);
		return;
    }
    
    //总体数据并无风险，不会导致服务器崩溃，要看看个体数据，找一下恶意者了    
    LPSTRUC_MSG_HEADER pMsgHeader = (LPSTRUC_MSG_HEADER)pcData;
	lpngx_connection_t pConn = pMsgHeader->pConn;
    if(pConn->iSendCount > 400)
    {
        //该用户收消息太慢【或者干脆不收消息】，累积的该用户的发送队列中有的数据条目数过大，认为是恶意用户，直接切断
       LOG_STDERR("发现某用户%d积压了大量待发送数据包，切断与他的连接！",pConn->fd);      
        m_iDiscardSendPkgCount++;
        pMemory->FreeMemory(pcData);
        KickConnection(pConn); //直接关闭
		return;
    }

    ++pConn->iSendCount; //发送队列中有的数据条目数+1；
    m_listSendBuff.push_back(pcData);     
    ++m_iListSendBuffCount;   //原子操作
...
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时02分11秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void CSocekt::KickConnection(lpngx_connection_t pConn)
{
    if(m_iIsOpenKickTimer == 1)
    {
        DeleteFromTimerQueue(pConn); //从时间队列中把连接干掉
    }
    ...
    InputConnection2RecyList(pConn);
...
返 回 值: 是否flood攻击,返回true;   否则返回false
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时02分33秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
bool CSocekt::TestFlood(lpngx_connection_t pConn)
{
	...
	if(pConn->iFloodAttackCount >= m_iFloodKickCount)
	{
		//可以踢此人的标志
		bRet = true;
	}
...
void CSocekt::PrintTDInfo()
{
    ...
    if( (currtime - m_timeLastPrint) > 10)
    {
        //超过10秒我们打印一次
        int tmprmqc = g_ThreadPool.getRecvMsgQueueCount(); //收消息队列

        m_timeLastPrint = currtime;
        int tmpoLUC = m_iOnlineUserCount;    //atomic做个中转，直接打印atomic类型报错；
        int tmpsmqc = m_iListSendBuffCount; //atomic做个中转，直接打印atomic类型报错；
       LOG_STDERR("------------------------------------begin--------------------------------------");
       LOG_STDERR("当前在线人数/总人数(%d/%d)。",tmpoLUC,m_iWorkerMaxConnection);        
       LOG_STDERR("连接池中空闲连接/总连接/要释放的连接(%d/%d/%d)。",m_listFreeConnection.size(),m_listConnection.size(),m_listRecyConnection.size());
       LOG_STDERR("当前时间队列大小(%d)。", m_mapTime.size());        
       LOG_STDERR("当前收消息队列/发消息队列大小分别为(%d/%d)，丢弃的待发送数据包数量为%d。",tmprmqc,tmpsmqc,m_iDiscardSendPkgCount);        
        if( tmprmqc > 100000)
        {
            //接收队列过大，报一下，这个属于应该 引起警觉的，考虑限速等等手段
           LOG_STDERR("接收队列条目数量过大(%d)，要考虑限速或者增加处理线程数量了！！！！！！",tmprmqc);
        }
       LOG_STDERR("-------------------------------------end---------------------------------------");
    }
    return;
...
/******************************************************************************************
函数原型: 
功能描述: (在子进程中)epoll功能初始化 
参数说明:   名称            类型                说明

返 回 值: 
依 赖 于: 
被引用于: static void NgxWorkerProcessInit(int iProcesseIndex)
创建日期: 2020年09月11日 10时03分39秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
int CSocekt::EpollInit()
{
    ...
    if (m_hEpoll == -1) 
    {
        LOG_STDERR1(errno,"CSocekt::EpollInit()中epoll_create()失败.");
    ...
    initconnection();
    /*
    m_connection_n = m_iWorkerMaxConnection;      //记录当前连接池中连接总数
    //连接池【数组，每个元素是一个对象】
    ...
    do 
    {
        ...
        //好从屁股往前来---------
        c[i].data = pNext;         //设置连接对象的next指针，注意第一次循环时next = NULL;
    ...
    m_pfree_connections = pNext;            //设置空闲连接链表头指针,因为现在next指向c[0]，注意现在整个链表都是空的
    m_iFreeConnection = m_connection_n;  //空闲连接链表长度，因为现在整个链表都是空的，这两个长度相等；
    */
    
    //(3)遍历所有监听socket【监听端口】，我们为每个监听socket增加一个 连接池中的连接【说白了就是让一个socket和一个内存绑定，以方便记录该sokcet相关的数据、状态等等】
    std::vector<lpngx_listening_t>::iterator pos;	
	for(pos = m_listListenSocket.begin(); pos != m_listListenSocket.end(); ++pos)
    {
        lpngx_connection_t pConn = GetConnectionFromFreeList((*pos)->fd); //从连接池中获取一个空闲连接对象
        if (pConn == NULL)
        {
            //这是致命问题，刚开始怎么可能连接池就为空呢？
            LOG_STDERR1(errno,"CSocekt::EpollInit()中ngx_get_connection()失败.");
            exit(2); //这是致命问题了，直接退，资源由系统释放吧，这里不刻意释放了，比较麻烦
        }
        pConn->pListening = (*pos);   //连接对象 和监听对象关联，方便通过连接对象找监听对象
        (*pos)->pConnection = pConn;  //监听对象 和连接对象关联，方便通过监听对象找连接对象

        //rev->accept = 1; //监听端口必须设置accept标志为1  ，这个是否有必要，再研究

        //对监听端口的读事件设置处理方法，因为监听端口是用来等对方连接的发送三路握手的，所以监听端口关心的就是读事件
        pConn->pmfRead = &CSocekt::OnAccept;
                                ...
                                )
{
    ...
    {
        LOG_STDERR1(errno,"CSocekt::ngx_epoll_add_event()中epoll_ctl(%d,%d,%d,%ud,%ud)失败.",fd,readevent,writeevent,otherflag,iEventType);
...
参数说明:   名称            类型                说明
            fd              int                 socket句柄
            iEventType      uint32_t            事件类型，一般是EPOLL_CTL_ADD，EPOLL_CTL_MOD，EPOLL_CTL_DEL ，说白了就是操作epoll红黑树的节点(增加，修改，删除)
            iEventFlag      uint32_t            标志，具体含义取决于eventtype
            iExtAction      int                 补充动作，用于补充flag标记的不足  :  0：增加   1：去掉 2：完全覆盖 ,eventtype是EPOLL_CTL_MOD时这个参数就有用
            pConn           lpngx_connection_t  pConn：一个指针【其实是一个连接】，EPOLL_CTL_ADD时增加到红黑树中去，将来epoll_wait时能取出来用

返 回 值: 成功返回1，失败返回-1；
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时04分05秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
int CSocekt::OperateEpollEvent(int fd, uint32_t iEventType, uint32_t iEventFlag, int iExtAction, lpngx_connection_t pConn )
{
    struct epoll_event ev;    
    memset(&ev, 0, sizeof(ev));

    if(iEventType == EPOLL_CTL_ADD) //往红黑树中增加节点；
    {
        //红黑树从无到有增加节点
        //ev.data.ptr = (void *)pConn;
    ...
    else if(iEventType == EPOLL_CTL_MOD)
    {
        ...
        {
            //增加某个标记            
            ev.events |= iEventFlag;
        }
        else if(iExtAction == 1)
        {
            //去掉某个标记
            ev.events &= ~iEventFlag;
        }
    ...
    if(epoll_ctl(m_hEpoll,iEventType,fd,&ev) == -1)
    {
        LOG_STDERR1(errno,"epoll_ctl(%d,%ud,%ud,%d)失败.",fd,iEventType,iEventFlag,iExtAction);    
        return -1;
    }
    return 1;
}
...
功能描述: 开始获取发生的事件消息
参数说明:   名称            类型                说明
            iTimeOut        int                 epoll_wait()阻塞的时长（毫秒）
返 回 值: 1：正常返回  ,0：有问题返回，一般不管是正常还是问题返回，都应该保持进程继续运行
依 赖 于: 
被引用于: 被 ngx_process_events_and_timers() 调用
          且 在子进程的死循环中被反复调用
创建日期: 2020年09月11日 10时04分42秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
int CSocekt::ProcessEpollEvents(int iTimeOut) 
{
    ...
    int events = epoll_wait(m_hEpoll,m_arrEvents,NGX_MAX_EVENTS,iTimeOut);    
    ...
    if(events == -1)
    {
        ...
        if(errno == EINTR) 
        {
            ...
            LOG_INFO("epoll_wait()失败.");
            return 1;  //正常返回
        }
        else
        {
            //这被认为应该是有问题，记录日志
            LOG_ALERT("epoll_wait()失败!");
            return 0;  //非正常返回 
        }
    }
    ...
    if(events == 0) //超时，但没事件来
    {
        ...
        LOG_ALERT("epoll_wait()没超时却没返回任何事件!");
        return 0; //非正常返回 
    }

    //会惊群，一个telnet上来，4个worker进程都会被惊动，都执行下边这个
    LOG_INFO("惊群测试:events=%d,进程id=%d",events,g_CurrPID);
    LOG_INFO("----------------------------------------"); 
    ...
    for(int i = 0; i < events; ++i)    //遍历本次epoll_wait返回的所有事件，注意events才是返回的实际事件数量
    {
        ...
        */
        ...
        if(revents & EPOLLIN)
        {// 是读事件
            //LOG_STDERR1(errno,"数据来了 ~~~~~~~~~~~~~.");
            //一个客户端新连入，这个会成立，
            //已连接发送数据来，这个也成立；
            //c->iReadReady = 1;                         //标记可以读；【从连接池拿出一个连接时这个连接的所有成员都是0】            
            (this->* (pConn->pmfRead) )(pConn);       //注意括号的运用来正确设置优先级，防止编译出错；【如果是个新客户连入
                                                      //如果新连接进入，这里执行的应该是CSocekt::OnAccept(c)】            
                                                      //如果是已经连入，发送数据到这里，则这里执行的应该是 CSocekt::OnRead()     
        }
        ...
        if(revents & EPOLLOUT) //如果是写事件【对方关闭连接也触发这个，再研究。。。。。。】，注意上边的 if(revents & (EPOLLERR|EPOLLHUP))  revents |= EPOLLIN|EPOLLOUT; 读写标记都给加上了
        {
            ...
            {
                (this->* (pConn->pmfWrite))(pConn);   //如果有数据没有发送完毕，由系统驱动来发送，则这里执行的应该是 CSocekt::OnWrite()
            }
        }
...
依 赖 于: 
被引用于: 
创建日期: 2020年09月11日 10时08分41秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void* CSocekt::ServerSendQueueThread(void* pvThreadData)
{    
    ...
    {
        //如果信号量值>0，则 -1(减1) 并走下去，否则卡这里卡着【为了让信号量值+1，可以在其他线程调用sem_post达到，实际上在CSocekt::PushData2SendBuff()调用sem_post就达到了让这里sem_wait走下去的目的】
        //******如果被某个信号中断，sem_wait也可能过早的返回，错误为EINTR；
        //整个程序退出之前，也要sem_post()一下，确保如果本线程卡在sem_wait()，也能走下去从而让本线程成功返回
        if(sem_wait(&pSocket->m_semEventSendBuff) == -1)
        {
            //失败？及时报告，其他的也不好干啥
            if(errno != EINTR) //这个我就不算个错误了【当阻塞于某个慢系统调用的一个进程捕获某个信号且相应信号处理函数返回时，该系统调用可能返回一个EINTR错误。】
                LOG_STDERR1(errno,"CSocekt::ServerSendQueueThread()中sem_wait(&pSocket->m_semEventSendBuff)失败.");            
        }

        //一般走到这里都表示需要处理数据收发了
        if(g_iStopEvent != 0)  //要求整个进程退出
            break;
        ...
        if(pSocket->m_iListSendBuffCount > 0) //原子的 
        {
            ...
            while(pos != posend)
            {
                ...
                pMsgHeader = (LPSTRUC_MSG_HEADER)pcMsgBuf;  //指向消息头
                pPkgHeader = (LPCOMM_PKG_HEADER)(pcMsgBuf+pSocket->m_iLenMsgHeader);	//指向包头
                pConn = pMsgHeader->pConn;

                //包过期，因为如果 这个连接被回收，比如在ngx_close_connection(),InputConnection2RecyList()中都会自增iCurrsequence
                ...
                //LOG_STDERR1(errno,"即将发送数据%ud。",pConn->iSendLen);

                lSendSize = pSocket->WriteData(pConn,pConn->pcSendBuff,pConn->iSendLen); //注意参数
                if(lSendSize > 0)
                {                    
                    if(lSendSize == pConn->iSendLen) //成功发送出去了数据，一下就发送出去这很顺利
                    {
                        ...
                        //LogStdErr(0,"CSocekt::ServerSendQueueThread()中数据发送完毕，很好。"); //做个提示吧，商用时可以干掉
                    }
                    else  //没有全部发送完毕(EAGAIN)，数据只发出去了一部分，但肯定是因为 发送缓冲区满了,那么
                    {                        
                        //发送到了哪里，剩余多少，记录下来，方便下次 sendproc() 时使用
                        pConn->pcSendBuff = pConn->pcSendBuff + lSendSize;
				        pConn->iSendLen = pConn->iSendLen - lSendSize;	
                        //因为发送缓冲区慢了，所以 现在我要依赖系统通知来发送数据了
                        ++pConn->iThrowsendCount;   //标记发送缓冲区满了，需要通过epoll事件来驱动消息的继续发送【原子+1，且不可写成p_Conn->iThrowsendCount = pConn->iThrowsendCount +1 ，这种写法不是原子+1】
                        //投递此事件后，我们将依靠epoll驱动调用ngx_write_request_handler()函数发送数据
                        if(pSocket->OperateEpollEvent(
                                pConn->fd,         //socket句柄
                                ...
                                ) == -1)
                        {
                            ...
                            LOG_STDERR1(errno,"CSocekt::ServerSendQueueThread()OperateEpollEvent()失败.");
                        }

                        //LOG_STDERR1(errno,"CSocekt::ServerSendQueueThread()中数据没发送完毕【发送缓冲区满】，整个要发送%d，实际发送了%d。",pConn->iSendLen,lSendSize);
                ...
                else if(lSendSize == 0)
                {
                    ...
                    //LOG_STDERR1(errno,"CSocekt::ServerSendQueueThread()中sendproc()居然返回0？"); //如果对方关闭连接出现send=0，那么这个日志可能会常出现，商用时就 应该干掉
                    //然后这个包干掉，不发送了
                    pMemory->FreeMemory(pConn->pcSendMemery);  //释放内存
                    pConn->pcSendMemery = NULL;
                    pConn->iThrowsendCount = 0;  //这行其实可以没有，因此此时此刻这东西就是=0的    
                ...
                else if(lSendSize == -1)
                {
                    ...
                    {
                        //有这情况发生？这可比较麻烦，不过先do nothing
                        LOG_STDERR1(errno,"CSocekt::ServerSendQueueThread()中ngx_epoll_add_event()_2失败.");
                    }
                    continue;
                }
