/******************************************************************************************
类    名: CSocekt
头 文 件: _include/ngx_c_socket.h
实现文件: 	net/ngx_c_socket.cxx
			net/ngx_c_socket_request.cxx
功能描述: socket相关类
依 赖 于: 
被引用于: 
创建日期: 2020年09月10日 13时55分07秒
修改记录: 
...
typedef void (CSocekt::*pmfEpollEventHandler)(lpngx_connection_t c);  // （用【::*】）定义类成员函数指针
...
struct ngx_listening_s  //和监听端口有关的结构
{
	int                 iPort;      //监听的端口号
	int                 fd;         //套接字句柄socket
	lpngx_connection_t  pConnection; //连接池中的一个连接，注意这是个指针 
};
...
{		
	...
	void InitBeforeUse();             //分配出去的时候初始化一些内容
	...
	//和读有关的标志-----------------------
	//uint8_t                   iReadReady;        //读准备好标记【暂时没闹明白官方要怎么用，所以先注释掉】
	//uint8_t                   iWriteReady;        //写准备好标记

	pmfEpollEventHandler      pmfRead;                       //读事件的相关处理方法
	pmfEpollEventHandler      pmfWrite;                       //写事件的相关处理方法

	//和epoll事件有关
	uint32_t                  events;                         //和epoll事件有关  
	
	//和收包有关
	unsigned char             cCurrStat;                        //当前收包的状态
	char                      szDataHeadInfo[_DATA_BUFSIZE_];   //用于保存收到的数据的包头信息			
	char                      *pcRecvBuff;                      //接收数据的缓冲区的头指针，对收到不全的包非常有用，看具体应用的代码
	unsigned int              iRecvLen;                       //要收到多少数据，由这个变量指定，和precvbuf配套使用，看具体应用的代码
	char                      *pcRecvMemery;               //new出来的用于收包的内存首地址，释放用的

	pthread_mutex_t           mutexLogicProcess;                 //逻辑处理相关的互斥量      

	//和发包有关
	std::atomic<int>          iThrowsendCount;  //发送消息，如果发送缓冲区满了，则需要通过epoll事件来驱动消息的继续发送，所以如果发送缓冲区满，则用这个变量标记
	char                      *pcSendMemery;    //发送完成后释放用的，整个数据的头指针，其实是 消息头 + 包头 + 包体
	char                      *pcSendBuff;      //发送数据的缓冲区的头指针，开始 其实是包头+包体
	unsigned int              iSendLen;         //要发送多少数据

	//和回收有关
	time_t                    timeInputRecy;    //入到资源回收站里去的时间
	...
	lpngx_connection_t        pNext;                //这是个指针，指向下一个本类型对象，用于把空闲的连接池对象串起来构成一个单向链表，方便取用
};
...
public:
	...
	virtual bool InitializeSubProc();  //初始化函数[子进程中执行]
	...
	//int  ngx_epoll_add_event(int fd,int readevent,int writeevent,uint32_t otherflag,uint32_t iEventType,lpngx_connection_t pConn);     
	                                                                      //epoll增加事件
...
protected:
	...
	void KickConnection(lpngx_connection_t pConn);  //主动关闭一个连接时的要做些善后的处理函数	
	
private:	
	void ReadConf();                    //专门用于读各种配置项	
	bool NgxOpenListeningSockets();     //监听必须的端口【支持多个端口】
	...
	ssize_t ReadData(lpngx_connection_t pConn,char *buff,ssize_t buflen); //读(socket)数据
    ssize_t WriteData(lpngx_connection_t c,char *buff,ssize_t size);      //写(socket)数据 

	void ngx_wait_request_handler_proc_p1(lpngx_connection_t pConn,bool &isflood);      //包头收完整后的处理，我们称为包处理阶段1
	void ngx_wait_request_handler_proc_plast(lpngx_connection_t pConn,bool &isflood);   //收到一个完整包后的处理，放到一个函数中，方便调用	
	void ClearSendBuffList();   //处理发送消息队列  

	//获取对端信息相关                                              
	size_t ngx_sock_ntop(struct sockaddr *sa,int iPort,u_char *text,size_t len);  //根据参数1给定的信息，获取地址端口字符串，返回这个字符串的长度

	//连接池 或 连接 相关
	void initconnection();  //初始化连接池
	void clearconnection(); //回收连接池
	lpngx_connection_t GetConnectionFromFreeList(int isock);    //从连接池中获取一个空闲连接
	void InputConnection2FreeList(lpngx_connection_t pConn);    //归还参数pConn所代表的连接到到连接池中	
	void InputConnection2RecyList(lpngx_connection_t pConn);    //将要回收的连接放到一个队列中来
	...
	//线程相关函数
	static void* ServerSendQueueThread(void *pvThreadData);         //专门用来发送数据的线程
	static void* ServerRecyConnectionThread(void *pvThreadData);    //专门用来回收连接的线程
	static void* ServerTimerQueueMonitorThread(void *pvThreadData); //时间队列监视线程，处理到期不发心跳包的用户踢出的线程
...
private:
	struct ThreadItem   
    {
        ...
        CSocekt     *m_pSocekt;		//记录线程池的指针	
        bool        m_bIsRunning;	//标记是否正式启动起来，启动起来后，才允许调用StopAll()来释放

        ThreadItem(CSocekt *pthis):m_pSocekt(pthis),m_bIsRunning(false){}                             
        ~ThreadItem(){}        
    };
	...
	std::list<lpngx_connection_t>  m_listFreeConnection;        //空闲连接列表【这里边装的全是空闲的连接】
	std::atomic<int>               m_iTotalConnection;          //连接池总连接数
	std::atomic<int>               m_iFreeConnection;           //连接池空闲连接数
	pthread_mutex_t                m_mutexConnection;           //连接相关互斥量，互斥m_freeconnectionList，m_connectionList
	pthread_mutex_t                m_mutexRecyConnQueue;        //连接回收队列相关的互斥量
	std::list<lpngx_connection_t>  m_listRecyConnection;        //将要释放的连接放这里
	std::atomic<int>               m_iTotolRecyConnection;      //待释放连接队列大小
	int                            m_iRecyConnectionWaitSeconds;//等待这么些秒后才回收连接
	...
	std::vector<lpngx_listening_t> m_listListenSocket;          //监听套接字队列
	struct epoll_event             m_arrEvents[NGX_MAX_EVENTS]; //用于在epoll_wait()中承载返回的所发生的事件

	//消息队列 
	std::list<char *>              m_listSendBuff;      //发送数据消息队列
	std::atomic<int>               m_iListSendBuffCount;//发消息队列大小
    pthread_mutex_t                m_mutexListSendBuff; //发消息队列互斥量

	//多线程相关
	std::vector<ThreadItem *>      m_vectorThread;          //线程容器 	
	sem_t                          m_semEventSendBuff;     //处理发消息线程相关的信号量 

	//时间相关
	int                            m_iIsOpenKickTimer;      //是否开启踢人时钟，1：开启   0：不开启		
	pthread_mutex_t                m_mutexTimeMap;          //和时间队列有关的互斥量
	std::multimap<time_t, LPSTRUC_MSG_HEADER>   m_mapTime;  //时间队列	
	size_t                         m_lTimeMaxCurrSize;      //时间队列的尺寸
	time_t                         m_timeValue;             //当前计时队列头部时间值
	...
	//统计用途
	time_t  m_timeLastPrint;         //上次打印统计信息的时间(10秒钟打印一次)
