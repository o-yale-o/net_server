...
/******************************************************************************************
函数原型: 
功能描述: 处理收到的数据包，由线程池来调用本函数，本函数是一个单独的线程
参数说明:   名称            类型                说明
            pcMsgBuf        char*               消息头 + 包头 + 包体 ：自解释
返 回 值: 
依 赖 于: 
被引用于: 
创建日期: 2020年09月14日 12时50分14秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void CLogicSocket::ProcessClientRequest(char *pcMsgBuf)
{          
    LPSTRUC_MSG_HEADER pMsgHeader = (LPSTRUC_MSG_HEADER)pcMsgBuf;                  //消息头
    LPCOMM_PKG_HEADER  pPkgHeader = (LPCOMM_PKG_HEADER)(pcMsgBuf+m_iLenMsgHeader); //包头
    ...
    else 
	{
		...
		pPkgBody = (void *)(pcMsgBuf+m_iLenMsgHeader+m_iLenPkgHeader); //跳过消息头 以及 包头 ，指向包体

        //LogStdErr(0,"CLogicSocket::ProcessClientRequest()中收到包的crc值为%d!",pPkgHeader->crc32);
		...
		{
           LOG_STDERR("CLogicSocket::ProcessClientRequest()中CRC错误[服务器:%d/客户端:%d]，丢弃数据!",calccrc,pPkgHeader->crc32);    //正式代码中可以干掉这个信息
			return; //crc错，直接丢弃
		}
        else
        {
            //LogStdErr(0,"CLogicSocket::ProcessClientRequest()中CRC正确[服务器:%d/客户端:%d]，不错!",calccrc,pPkgHeader->crc32);
        }        
	}

    //包crc校验OK才能走到这里    	
    unsigned short imsgCode = ntohs(pPkgHeader->msgCode); //消息代码拿出来
    lpngx_connection_t pConn = pMsgHeader->pConn;        //消息头中藏着连接池中连接的指针

    //我们要做一些判断
    //(1)如果从收到客户端发送来的包，到服务器释放一个线程池中的线程处理该包的过程中，客户端断开了，那显然，这种收到的包我们就不必处理了；    
    if(pConn->iCurrSequence != pMsgHeader->iCurrSequence)   //该连接池中连接以被其他tcp连接【其他socket】占用，这说明原来的 客户端和本服务器的连接断了，这种包直接丢弃不理
    {
    ...
    {
       LOG_STDERR("CLogicSocket::ProcessClientRequest()中imsgCode=%d消息码不对!",imsgCode); //这种有恶意倾向或者错误倾向的包，希望打印出来看看是谁干的
    ...
    {
       LOG_STDERR("CLogicSocket::ProcessClientRequest()中imsgCode=%d消息码找不到对应的处理函数!",imsgCode); //这种有恶意倾向或者错误倾向的包，希望打印出来看看是谁干的
        return;  //没有相关的处理函数
    }
...
/******************************************************************************************
函数原型: 
功能描述: 心跳包检测时间到，该去检测心跳包是否超时的事宜，本函数是子类函数，实现具体的判断动作
参数说明:   名称            类型                说明

返 回 值: 
依 赖 于: 
被引用于: 
创建日期: 2020年09月14日 12时51分09秒
修改记录: 
        修改日期    修改人          修改标记        新版本号    修改原因
******************************************************************************************/
void CLogicSocket::ProcessIdleTimeOut(LPSTRUC_MSG_HEADER tmpmsg,time_t cur_time)
{
    ...
    if(tmpmsg->iCurrSequence == tmpmsg->pConn->iCurrSequence) //此连接没断
    {
        ...
        else if( (cur_time - pConn->timeLastPing ) > (m_iWaitTime*3+10) ) //超时踢的判断标准就是 每次检查的时间间隔*3，超过这个时间没发送心跳包，就踢【大家可以根据实际情况自由设定】
        {
            //踢出去【如果此时此刻该用户正好断线，则这个socket可能立即被后续上来的连接复用  如果真有人这么倒霉，赶上这个点了，那么可能错踢，错踢就错踢】            
            //LogStdErr(0,"时间到不发心跳包，踢出去!");   //感觉OK
            KickConnection(pConn); 
        }   
             
        pMemory->FreeMemory(tmpmsg);//内存要释放
    }
    else //此连接断了
    {
        pMemory->FreeMemory(tmpmsg);//内存要释放
...
void CLogicSocket::SendNoBodyPkgToClient(LPSTRUC_MSG_HEADER pMsgHeader,unsigned short iMsgCode)
{
    ...
    char *p_sendbuf = (char *)pMemory->AllocMemory(m_iLenMsgHeader+m_iLenPkgHeader,false);
...
bool CLogicSocket::_HandleRegister(lpngx_connection_t pConn,LPSTRUC_MSG_HEADER pMsgHeader,char *pPkgBody,unsigned short iBodyLength)
{
    ...
    CLock lock(&pConn->mutexLogicProcess); //凡是和本用户有关的访问都互斥
    ...
    char *p_sendbuf = (char *)pMemory->AllocMemory(m_iLenMsgHeader+m_iLenPkgHeader+iSendLen,false);//准备发送的格式，这里是 消息头+包头+包体
    //b)填充消息头
    ...
    {
       LOG_STDERR("1111111111111111111111111111111111111111111111111111111111111!");
    } */
    ...
    {
        //应该是不等，因为这个插座已经被回收了
       LOG_STDERR("插座不等,%L--%L",pMsgHeader->iCurrSequence,pConn->iCurrSequence);
    }
    else
    {
       LOG_STDERR("插座相等哦,%L--%L",pMsgHeader->iCurrSequence,pConn->iCurrSequence);
    }
    
    */
    //LogStdErr(0,"执行了CLogicSocket::_HandleRegister()并返回结果!");
    return true;
}
...
bool CLogicSocket::_HandlePing(lpngx_connection_t pConn,LPSTRUC_MSG_HEADER pMsgHeader,char *pPkgBody,unsigned short iBodyLength)
{
    ...
    CLock lock(&pConn->mutexLogicProcess); //凡是和本用户有关的访问都考虑用互斥，以免该用户同时发送过来两个命令达到各种作弊目的
    pConn->timeLastPing = time(NULL);   //更新该变量

    //服务器也发送 一个只有包头的数据包给客户端，作为返回的数据
    SendNoBodyPkgToClient(pMsgHeader,_CMD_PING);

    //LogStdErr(0,"成功收到了心跳包并返回结果！");
    return true;
}